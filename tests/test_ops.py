...
def test_purge_main(tmp_path):
    ...
    with (
        patch(
            "sys.argv",
            ["jbot-purge.py", "-d", str(dir_path), "-a", str(dir_path / "archive")],
        ),
        patch(
            "jbot_utils.read_file",
            side_effect=lambda p, d="": open(p).read() if os.path.exists(p) else d,
        ),
    ):
        jbot_purge.main()
...
def test_rotate_memory_main(tmp_path):
    ...
    with patch(
        "sys.argv",
        ["jbot-rotate.py", "-m", str(log_file), "-a", str(archive_log), "-line", "10"],
    ):
        jbot_rotate.main()
...
def test_rotate_messages_main(tmp_path):
    ...
    msgs_left = [
        f for f in os.listdir(msgs_dir) if os.path.isfile(os.path.join(msgs_dir, f))
    ]
