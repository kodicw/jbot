...
def test_cli_version(tmp_path, capsys):
    ...
    with patch("jbot_utils.bump_version", return_value=None):
        with patch(
            "sys.argv", ["jbot-cli.py", "-d", str(tmp_path), "version", "bump", "patch"]
        ):
            jbot_cli.main()
        captured = capsys.readouterr()
...
def test_cli_infrastructure_commands(tmp_path, capsys):
    ...
    with (
        ...
        patch("jbot_utils.purge_directives", return_value=5),
        patch("jbot_utils.rotate_memory", return_value=True),
        patch("jbot_utils.rotate_tasks", return_value=True),
        patch("jbot_utils.rotate_messages", return_value=True),
        ...
        # maintenance
        with patch("sys.argv", ["jbot-cli.py", "-d", str(tmp_path), "maintenance"]):
            jbot_cli.main()

        # purge
        with patch("sys.argv", ["jbot-cli.py", "-d", str(tmp_path), "purge"]):
            jbot_cli.main()
        assert "Purged 5 expired directives." in capsys.readouterr().out

        # rotate memory
        with patch(
            "sys.argv", ["jbot-cli.py", "-d", str(tmp_path), "rotate", "memory"]
        ):
            jbot_cli.main()
        assert "Memory log rotated." in capsys.readouterr().out

        # rotate tasks
        with patch("sys.argv", ["jbot-cli.py", "-d", str(tmp_path), "rotate", "tasks"]):
            jbot_cli.main()
        assert "Tasks rotated." in capsys.readouterr().out

        # rotate messages
        with patch(
            "sys.argv", ["jbot-cli.py", "-d", str(tmp_path), "rotate", "messages"]
        ):
            jbot_cli.main()
        assert "Messages rotated." in capsys.readouterr().out

        # dashboard
        with patch("sys.argv", ["jbot-cli.py", "-d", str(tmp_path), "dashboard"]):
            jbot_cli.main()
