...
def test_billing_data(tmp_path):
    jbot_dir = tmp_path / ".jbot"
    jbot_dir.mkdir()
    
    # Test default
    data = utils.get_billing_data(str(tmp_path))
    assert data["total_cost"] == 0.0
    
    # Test update
    utils.update_billing_data(str(tmp_path), 0.05)
    data = utils.get_billing_data(str(tmp_path))
...
def test_get_recent_logs(tmp_path):
    ...
    log_file.write_text(json.dumps({"agent": "a1", "content": {"summary": "s1"}}) + "\n")
...
def test_parse_directives(tmp_path):
    ...
    (dir_path / "003_expired_content.txt").write_text("Expired content\nExpiration: 2000-01-01")
