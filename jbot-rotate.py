...
def rotate_memory(memory_log=".jbot/memory.log", archive_log=".jbot/memory.log.archive", limit=100):
    ...
    try:
        ...
        if len(lines) <= limit:
            log(f"Memory log has {len(lines)} entries (Limit: {limit}). No rotation needed.")
            return
        ...
        log(f"Rotating memory: Keeping {len(to_keep)} entries, Archiving {len(to_archive)} entries.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--memory", default=".jbot/memory.log", help="Memory log file")
    parser.add_argument("-a", "--archive", default=".jbot/memory.log.archive", help="Archive log file")
    parser.add_argument("-l", "--limit", type=int, default=100, help="Max entries to keep in memory.log")
