...
def rotate_messages(msgs_dir=".jbot/messages", archive_dir=".jbot/messages/archive", limit=20):
    ...
    all_msgs = sorted([f for f in os.listdir(msgs_dir) if f.endswith(".txt") and f != "human.txt"])
    
    if len(all_msgs) <= limit:
        log(f"Messages directory has {len(all_msgs)} entries (Limit: {limit}). No rotation needed.")
        return
    ...
    log(f"Rotating messages: Keeping {len(to_keep)} entries, Archiving {len(to_archive)} entries.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--messages", default=".jbot/messages", help="Messages directory")
    parser.add_argument("-a", "--archive", default=".jbot/messages/archive", help="Archive directory")
    parser.add_argument("-l", "--limit", type=int, default=20, help="Max entries to keep in messages/")
