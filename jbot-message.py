...
def send_message(to_dir, agent_name, body, subject="No Subject"):
    ...
    if not os.path.exists(msgs_dir):
        log(f"Error: Messages directory {msgs_dir} not found. Ensure the project is initialized.")
        return False
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dir", default=".", help="Project directory to send the message to (default: current directory)")
    parser.add_argument("-f", "--from-agent", required=True, help="Name of the agent sending the message")
    ...
    send_message(to_dir=args.dir, agent_name=args.from_agent, body=args.message, subject=args.subject)
